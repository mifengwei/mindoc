...
func AddMember(account, password string) error {
	...
	if err == nil {
		...
		if err != nil {
			logger.Error("PasswordModify => ", err)
			return err
		}
		return nil
	}
...
func ModifyPassword(account, old_password, new_password string) error {
	...
	if err != nil {
		logger.Error(fmt.Sprintf("Password could not be changed: %s", err.Error()))
		return err
	}
	return nil
}
